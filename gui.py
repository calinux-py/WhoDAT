...
from PyQt5.QtWidgets import (
    QMainWindow, QApplication, QTabWidget, QWidget, QVBoxLayout, QTextEdit,
    QFormLayout, QLabel, QLineEdit, QPushButton, QTextBrowser,
...
class MainWindow(QMainWindow):
    ...
    def make_urlscan_links(self, text):

        pattern = r'(https?://urlscan\.io/screenshots/[^\s<>"]+)'
    ...
    def create_domain_tab(self):
        ...
        self.output_text.setTextInteractionFlags(Qt.TextBrowserInteraction)
    ...
    def create_header_tab(self):
        ...
        self.header_output_text = QTextBrowser()
        self.header_output_text.setReadOnly(True)
        self.header_output_text.setFont(QFont("Segoe UI", 11))

        self.header_output_text.setOpenExternalLinks(True)
        self.header_output_text.setTextInteractionFlags(Qt.TextBrowserInteraction)
        self.header_output_text.setAcceptRichText(True)
    ...
    def create_sentiment_tab(self):
        ...
        self.sentiment_output_text = QTextBrowser()
        self.sentiment_output_text.setReadOnly(True)
        self.sentiment_output_text.setFont(QFont("Segoe UI", 11))

        self.sentiment_output_text.setOpenExternalLinks(True)
        self.sentiment_output_text.setTextInteractionFlags(Qt.TextBrowserInteraction)
        self.sentiment_output_text.setAcceptRichText(True)
    ...
    def create_attachment_tab(self):
        ...
        self.attachment_output_text = QTextBrowser()
        self.attachment_output_text.setReadOnly(True)
        self.attachment_output_text.setFont(QFont("Segoe UI", 11))

        self.attachment_output_text.setOpenExternalLinks(True)
        self.attachment_output_text.setTextInteractionFlags(Qt.TextBrowserInteraction)
        self.attachment_output_text.setAcceptRichText(True)
    ...
    def analyze(self):
        ...
        if not email_input and not link:
            ...
            self.output_text.append(
                "<i><span style='color:lightgrey;'>Skipping analysis because no input was provided.</span></i><br>")
            return
    ...
    def analyze_sentiment(self):
        ...
        if not openai_api_key:
            ...
            self.sentiment_output_text.append(
                "<i><span style='color:lightgrey;'>Skipping Sentiment Analysis because OpenAI API key is not provided.</span></i><br>")
            return
    ...
    def browse_file(self):
        ...
        if file_path:
            ...
            if not vt_api_key:
                ...
                self.attachment_output_text.append(
                    "<i><span style='color:lightgrey;'>Skipping Attachment Analysis because VirusTotal API key is not provided.</span></i><br>")
                return
            if not ha_api_key:
                QMessageBox.warning(self, "API Key Error", "Hybrid Analysis API key not provided in config.")
                self.attachment_output_text.append(
                    "<i><span style='color:lightgrey;'>Skipping Hybrid Analysis because Hybrid Analysis API key is not provided.</span></i><br>")
    ...
    def append_output(self, text):
        if "Processing... Please wait." in self.output_text.toHtml():
            self.output_text.clear()
        linked_text = self.make_urlscan_links(text)
        self.output_text.append(linked_text)

    def show_error(self, message):
        QMessageBox.warning(self, "Error", message)

    def append_header_output(self, text):
        if "Processing... Please wait." in self.header_output_text.toHtml():
            self.header_output_text.clear()
        linked_text = self.make_urlscan_links(text)
        self.header_output_text.append(linked_text)
    ...
    def show_header_error(self, message):
        ...
        self.header_output_text.append(
            "<i><span style='color:lightgrey;'>Skipping Header Analysis due to an error.</span></i><br>")
        self.header_input.clear()

    def append_sentiment_output(self, text):
        if "Processing... Please wait." in self.sentiment_output_text.toHtml():
            self.sentiment_output_text.clear()
        linked_text = self.make_urlscan_links(text)
        self.sentiment_output_text.append(linked_text)

    def show_sentiment_error(self, message):
        QMessageBox.warning(self, "Error", f"Sentiment Analysis Error: {message}")
        self.sentiment_output_text.append(
            "<i><span style='color:lightgrey;'>Skipping Sentiment Analysis due to an error.</span></i><br>")
        self.sentiment_input.clear()

    def append_attachment_output(self, text):
        if "Processing... Please wait." in self.attachment_output_text.toHtml():
            self.attachment_output_text.clear()
        linked_text = self.make_urlscan_links(text)
        self.attachment_output_text.append(linked_text)

    def show_attachment_error(self, message):
        QMessageBox.warning(self, "Error", f"Attachment Analysis Error: {message}")
        self.attachment_output_text.append(
            "<i><span style='color:lightgrey;'>Skipping Attachment Analysis due to an error.</span></i><br>")
